...
public:
    ...
    PCB_RR_MLQ(int _PID, string _name, bool _cpu_bound, int _priority, int _arrival_time, int _burst_time, int _io_burst, int _burst_time2)
    {
        PID = _PID;
        name = _name;
...
bool readyQueueComparator(PCB_RR_MLQ p1, PCB_RR_MLQ p2)
{
...
{
public:
    vector<PCB_RR_MLQ> ready_queue;
    queue<PCB_RR_MLQ> working_queue, running_queue;
    vector<PCB_RR_MLQ> terminated;
    int current_time;
    ...
    void printQueue(vector<PCB_RR_MLQ> queue)
    {
        for (PCB_RR_MLQ pcb : queue)
        {
    ...
    void addNewProcessIfAny()
    {
        ...
        {
            PCB_RR_MLQ new_pcb = ready_queue.front();
    ...
    void performTasks()
    {
        while (!ready_queue.empty() || !running_queue.empty())
        {
            ...
            PCB_RR_MLQ running_top = running_queue.front();
            running_queue.pop();
            int working_time;
    ...
    void save_schedule(bool is_append)
    {
        ofstream fout;
        if (is_append)
            fout.open("output/MLQ_Schedule.csv", ios::in | ios::out | ios::app);
        else
        {
            fout.open("output/MLQ_Schedule.csv");
            fout << "id"
                 << ","
                 << "name"
                 << ","
